...
export default function AnswerModal(props: AnswerModalProps) {
  ...
  const validateForm = () => {
    // This validates the form information
    if (
      userName.length < 60 &&
      answer.length < 1000 &&
    ...
    if (answer.length > 1000)
      alert("Your answer must be under 1000 characters.");
  ...
  const postRequest = () => {
    //axios post request
    axios
      .request(postAnswer(props.question_id, answer, userName, email, pictures))
      .catch((error) =>
        console.log("Error in AnswerModal post request", error)
      );
  };
  //this the the end of form validation and post

  const handleClose = () => {
    // closes modal not the problem
    props.setAnswerModal(false);
  };
  const conditionalPictures = useCallback(() => {
    // will show your pictures if you uploaded any
